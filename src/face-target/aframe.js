const {Controller, UI} = window.dybo.FACE;

const THREE = AFRAME.THREE;

AFRAME.registerSystem('dybo-face-system', {
  container: null,
  video: null,
  shouldFaceUser: true,
  lastHasFace: false,
...
AFRAME.registerComponent('dybo-face', {
  dependencies: ['dybo-face-system'],
  ...
  init: function() {
    const arSystem = this.el.sceneEl.systems['dybo-face-system'];

    if (this.data.faceOccluder) {
      const faceOccluderMeshEntity = document.createElement('a-entity');
      faceOccluderMeshEntity.setAttribute("dybo-face-default-face-occluder", true);
...
AFRAME.registerComponent('dybo-face-target', {
  dependencies: ['dybo-face-system'],
...
AFRAME.registerComponent('dybo-face-occluder', {
  init: function() {
    const root = this.el.object3D;
...
AFRAME.registerComponent('dybo-face-default-face-occluder', {
  init: function() {
    const arSystem = this.el.sceneEl.systems['dybo-face-system'];
